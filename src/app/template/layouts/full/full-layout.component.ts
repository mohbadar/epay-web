...
import { ConfigService } from "app/template/shared/services/config.service";
import { DOCUMENT } from "@angular/common";
import { Subscription } from "rxjs";
import { CustomizerService } from 'app/template/shared/services/customizer.service';
import { DeviceDetectorService } from 'ngx-device-detector';
import { LayoutService } from 'app/template/shared/services/layout.service';
import { WINDOW } from 'app/template/shared/services/window.service';
...
export class FullLayoutComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    ngOnInit() {
        this.route.data.subscribe(data => {
            this.hRoutes = data.HROUTES;
            this.vRoutes = data.VROUTES;
        });
        this.getConfig();
    }
    ...
    getConfig() {
        this.render = false;
        this.authService.getConfig().subscribe(response => {
            ...
            if (response.authenticated) {
                ...
                this.principal = new Principal(response.authenticated,
                    response.authorities, [], response.environments,
                    response.currentEnv, response.currentLang, response.name, response.username
                    , response.entity, response.entityId, response.department, response.departmentId, response.id, response.reviewerRequired, response.accessableDepartments);

                this.authService.principal = this.principal;
                this.globals.principal = this.principal;
                this.globals.preferences = response.envPreferences == null ? new Preferences(true, false, false, "red", "../../../assets/img/full-screen-image-3.jpg", 10) : response.envPreferences;
                ...
                console.log("Config fetched from server. env is null");
                // this.authService.routeToPreferencePage();
                this.authService.routeToLoginPage(true);
            }
            this.cdr.detectChanges();
        }, err => {
            console.log("Config fetched from server - failed");
            // this.authService.routeToPreferencePage();
            this.authService.routeToLoginPage(true);
        });
    }
