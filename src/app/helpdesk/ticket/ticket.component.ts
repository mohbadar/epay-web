...
export class TicketComponent implements OnInit {
	...
	constructor(private cdref: ChangeDetectorRef, public translate: TranslateService,
		...
		private sysRegService: SystemRegistryService, private ticketService: TicketService) { }
	...
	renderData(tableOptions, filters) {
		...
		this.spinner.show();
		this.ticketService.getRecordList(tableOptions, filters).subscribe((data:any) => {
			if (data == null) {
				this.rows = [];
				this.recordsTotal = 0;
			} else {
	...
	getTableOptions() {
		return {
			...
			"columns": [
				...
				{
					"data": "tic.title", "name": "TITLE",
					"searchable": false, "orderable": true,
					"search": { "value": "", "regex": false }
				},
				{
					"data": "tic.status", "name": "STATUS",
					"searchable": false, "orderable": true,
					"search": { "value": "", "regex": false }
				},
				{
					"data": "serv_type.name as SERVICE_TYPE", "name": "SERVICE_TYPE",
					"searchable": false, "orderable": true,
					"search": { "value": "", "regex": false }
				},
				{
