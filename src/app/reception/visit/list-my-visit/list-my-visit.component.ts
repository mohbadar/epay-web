...
@Component({
  selector: 'app-list-my-visit',
  templateUrl: './list-my-visit.component.html',
  styleUrls: ['./list-my-visit.component.scss']
})
export class ListMyVisitComponent implements OnInit {
	...
	constructor(private cdref: ChangeDetectorRef, public translate: TranslateService,
		...
		private sysRegService: SystemRegistryService, private visitService: VisitService,
	...
	renderData(tableOptions, filters) {
		...
		this.spinner.show();
		this.visitService.getMyRecordList(tableOptions, filters).subscribe((data:any) => {
			if (data == null) {
				this.rows = [];
				this.recordsTotal = 0;
			} else {
	...
	addNewScheduleRecord() {
		this.router.navigate(['reception/visits/schedules/add']);
		this.loading = true;
	}
  ...
  printMyVisitsList(id) {
    this.visitService.printMyVisitsList(id);
  }
