...
@Component({
  selector: 'list-schedule-visit',
  templateUrl: './list-schedule-visit.component.html',
  styleUrls: ['./list-schedule-visit.component.scss']
})
export class ListScheduleVisitComponent implements OnInit {
	...
	constructor(private cdref: ChangeDetectorRef, public translate: TranslateService,
		...
		private sysRegService: SystemRegistryService, private receptionService: ReceptionService,
	...
	renderData(tableOptions, filters) {
		...
		this.spinner.show();
		this.receptionService.getScheduleVisitList(tableOptions, filters).subscribe((data:any) => {
			if (data == null) {
				this.rows = [];
				this.recordsTotal = 0;
			} else {
