...
import { ProcessVisitorScheduleComponent } from '../process-visitor-schedule/process-visitor-schedule.component';

@Component({
  selector: 'list-schedule-visitor',
  templateUrl: './list-schedule-visitor.component.html',
  styleUrls: ['./list-schedule-visitor.component.scss']
})
export class ListScheduleVisitorComponent implements OnInit {
	@Input() interval;
	...
	renderData(tableOptions, filters) {
		...
		this.spinner.show();
		this.schedulingService.getScheduleVisitorList(tableOptions, filters, this.interval).subscribe((data:any) => {
			if (data == null) {
				this.rows = [];
				this.recordsTotal = 0;
			} else {
	...
	processRecord(row) {
		...
		const modalRef = this.modalService.open(ProcessVisitorScheduleComponent);
