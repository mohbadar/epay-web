...
@Component({
    selector: 'app-list-followup-document',
    templateUrl: './list-followup-document.component.html',
    styleUrls: ['./list-followup-document.component.scss']
})
export class ListFollowupDocumentComponent implements OnInit {
    ...
    renderData(tableOptions, filters) {
        ...
        this.spinner.show();
        this.documentService.getFollowupRecordList(tableOptions, filters).subscribe((data: any) => {
            if (data == null) {
                this.rows = [];
                this.recordsTotal = 0;
            } else {
