...
@Component({
    selector: 'app-list-draft-document',
    templateUrl: './list-draft-document.component.html',
    styleUrls: ['./list-draft-document.component.scss']
})
export class ListDraftDocumentComponent implements OnInit {
    ...
    renderData(tableOptions, filters) {
        ...
        this.spinner.show();
        this.draftDocumentService.getMyDocumentsList(tableOptions, filters).subscribe((data: any) => {
            if (data == null) {
                this.rows = [];
                this.recordsTotal = 0;
            } else {
