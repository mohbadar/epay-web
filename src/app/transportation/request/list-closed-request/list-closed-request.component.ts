...
@Component({
  selector: 'list-closed-request',
  templateUrl: './list-closed-request.component.html',
  styleUrls: ['./list-closed-request.component.scss']
})
export class ListClosedRequestComponent implements OnInit {
  ...
  fetchEssentialData() {
	...
	renderData(tableOptions, filters) {
		...
		this.spinner.show();
		this.requestService.getClosedRecordList(tableOptions, filters).subscribe((data:any) => {
			if (data == null) {
				this.rows = [];
				this.recordsTotal = 0;
			} else {
