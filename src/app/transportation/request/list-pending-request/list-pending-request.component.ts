...
@Component({
  selector: 'list-pending-request',
  templateUrl: './list-pending-request.component.html',
  styleUrls: ['./list-pending-request.component.scss']
})
export class ListPendingRequestComponent implements OnInit {
	...
	renderData(tableOptions, filters) {
		...
		this.spinner.show();
		this.requestService.getPendingRecordList(tableOptions, filters).subscribe((data:any) => {
			if (data == null) {
				this.rows = [];
				this.recordsTotal = 0;
			} else {
	...
	processRecord(row) {
		...
		const modalRef = this.modalService.open(ProcessRequestComponent);
