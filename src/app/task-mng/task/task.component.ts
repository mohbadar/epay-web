...
export class TaskComponent implements OnInit {
	...
	constructor(private cdref: ChangeDetectorRef, public translate: TranslateService,
		...
		private sysRegService: SystemRegistryService, private taskService: TaskService,
	...
	renderData(tableOptions, filters) {
		...
		this.spinner.show();
		this.taskService.getRecordList(tableOptions, filters).subscribe((data:any) => {
			if (data == null) {
				this.rows = [];
				this.recordsTotal = 0;
			} else {
	...
	getTableOptions() {
		return {
			...
			"columns": [
				...
				{
					"data": "tsk.progress", "name": "PROGRESS",
