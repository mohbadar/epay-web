...
export class EmployeeComponent implements OnInit {
	...
	constructor(private cdref: ChangeDetectorRef, public translate: TranslateService,
		...
		private sysRegService: SystemRegistryService, private employeeService: EmployeeService) { }
	...
	renderData(tableOptions, filters) {
		...
		this.spinner.show();
		this.employeeService.getRecordList(tableOptions, filters).subscribe((data:any) => {
			if (data == null) {
				this.rows = [];
				this.recordsTotal = 0;
			} else {
	...
	getTableOptions() {
		return {
			...
			"columns": [
				...
				{
					"data": "emp.first_name as NAME", "name": "NAME",
					"searchable": false, "orderable": true,
					"search": { "value": "", "regex": false }
				},
